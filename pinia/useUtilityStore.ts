...
import {dataTop20, fotosArticulosInterface} from "@/types/interfaces/CategoriaInterface";
...
export const useUtilityStore = defineStore("utility", () => {
  ...
  // handle Open Modal
  const handleOpenModal = (id: string, item: dataTop20) => {
    modalId.value = id;
    product.value = item;
    productStore.handleImageActive(item.url)
    productStore.changeColorImg(item.fotos[0].color);
    productStore.imgsSeleccionadasXColor(item.fotos);
